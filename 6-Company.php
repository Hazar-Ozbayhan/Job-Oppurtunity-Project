    ...
    <h4> Welcome <h4>
            <h4>
                ...
                if (isset($_POST['jid'])) {  # check if user selected any value 
                    ...
                    $query = "select username, applyDate , resumee, univ, program, gpa, standing, numDays
                              from internshipjobposting i , application a
                            where i.jid = a.jid";
                    ...
                    while ($index < $num) {
                        ...
                        $numx = $row["num"];   #return column value

                        echo $numx, "\n", "<br>";  #output
                        $index++;
                    }
                } else {
                    echo "Please choose a jid "; #print if else condition does not activating / when no data is submitted
                }
                ?> 
                <form method="post" action="6-Company.php">  <!-- post values to itself -->
