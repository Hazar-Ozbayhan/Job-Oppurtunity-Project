...
<body style="text-align:center;">
   ...
   <?php    
    if(isset($_POST['username'])){ # check if user selected any value 
        ...
        $Eusername = $_POST['username']; # take posted value from the form below
        $query= " select E.lname
                     from end_user E
        ...
        ?> 
                <form method="post" action="41-Company.php">  <!-- post values to itself -->
                    <select name ="username"> <!-- post username to istself -->
                        <?php
        include './dbconnection.php'; #establish connection        
        $queryx="select username from end_user ;"; #select department names to show
